...
class TenderEvaluationCrew:
    ...
    def __init__(self, tender_id: str, api_key: Optional[str] = None):
        load_dotenv()
    ...
    def run_full_evaluation(self, document_list: List[str], vendor_dossiers: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if self.api_key and self.api_key != "MISTRAL_API_KEY":
            try:
                ...
                return self._run_crewai_crew(document_list, vendor_dossiers)
            except Exception as e:
                # Step 2B: Fallback to rule engine if API error occurs
                logger.exception(f"CrewAI execution failed: {e}. Falling back to deterministic evaluation engine.")
                return self._run_deterministic_fallback(document_list, vendor_dossiers)
        else:
    ...
    def _run_crewai_crew(self, document_list: List[str], vendor_dossiers: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Assembles and runs CrewAI crew."""
        ...
        # Synthesize with local service output for type-safety
        from services.comparison_service import ComparisonService
        service = ComparisonService()
        result_data = service.generate_comparison_matrix(self.tender_id, vendor_dossiers)
        result_data["crewai_output"] = str(result)
        return result_data

    def _run_deterministic_fallback(self, document_list: List[str], vendor_dossiers: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Runs rule-based structured analysis engine directly."""
        try:
            from services.comparison_service import ComparisonService
            service = ComparisonService()
            result = service.generate_comparison_matrix(self.tender_id, vendor_dossiers)
